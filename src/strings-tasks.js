...
function findLongestWord(sentence) {
  return sentence.split(' ').sort(function sortLength(a, b) {
...
function reverseWords(str) {
  ...
  for (let i = 0; i < wordStr.length; i += 1) {
    words.push(wordStr[i].split('').reverse().join(''));
  }
  return words.join(' ');
}
...
function invertCase(str) {
  const word = [];
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toLowerCase()) word.push(str[i].toUpperCase());
    else if (str[i] === str[i].toUpperCase()) word.push(str[i].toLowerCase());
...
function encodeToRot13(str) {
  return str.replace(
    ...
    (position) =>
      'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'[
        'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.indexOf(position)
      ]
  );
}
